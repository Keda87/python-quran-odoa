...
def get_random_surah(lang='id'):
    ...
    random_pages = random.randint(1, TOTAL_PAGES)
    surah_url = '{base}/surah/surah_{pages}.json'.format(base=BASE_API,
                                                         pages=random_pages)
    ...
    try:
        ...
        translation = get_translation(surah=data.get('index'), ayat=ayat_key, lang=lang)

        return {'ayat': ayat, 'translate': translation}


def get_translation(surah, ayat, lang):
    """
    ...
    translation_url = '{base}/translations/{lang}/{lang}_translation_{surah}.json'\
                      .format(base=BASE_API, lang=lang, surah=int(surah))
